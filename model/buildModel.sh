...
PROTO_PATH="${PROTO_PATH:-../../../.nuget/packages/google.protobuf.tools/3.22.1/tools}"
...
mkdir -p ../tools/out/csharp
mkdir -p ../tools/out/java
mkdir -p ../tools/out/go
mkdir -p ../tools/out/js
mkdir -p ../tools/out/ts

#you will need to adjust the relative path to the protoc and grpc tools.

protoc --csharp_out=../tools/out/csharp --java_out=../tools/out/java --ts_out=import_style=commonjs:../tools/out/js --ts_out=import_style=commonjs:../tools/out/ts --grpc-web_out=import_style=commonjs,mode=grpcwebtext:../tools/out/js  --proto_path=. --proto_path=$PROTO_PATH ./metadata.proto --plugin=protoc-gen-web --plugin=protoc-gen-ts=$PROTOC_GEN_TS_PATH


cp ../tools/out/csharp/* ../tools/CC-Metadata/Model
